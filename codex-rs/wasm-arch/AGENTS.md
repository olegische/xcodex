...
For long-term maintainability, keep `codex-rs/wasm-arch/core` as close to `codex-rs/core` as possible.

- Prefer direct reuse from `codex-rs/core`.
- If reuse is not practical because of WASM or browser boundaries, copy the native logic as directly as possible into `codex-rs/wasm-arch/core`.
