...
export class AppServerBrowserRuntime implements BrowserRuntime {
  ...
  constructor(runtime: WasmProtocolRuntime) {
  ...
  private async startPump() {
    while (true) {
      ...
      if (isJsonRpcRequest(raw)) {
        console.info("[webui] app-server:request", summarizeJsonRpcRequest(raw));
        console.info("[webui] app-server:request:json", JSON.stringify(raw, null, 2));
        void this.handleServerRequest(raw);
        continue;
      }
      if (isJsonRpcNotification(raw)) {
        console.info("[webui] app-server:notification", summarizeJsonRpcNotification(raw));
        console.info("[webui] app-server:notification:json", JSON.stringify(raw, null, 2));
        this.handleServerNotification(raw);
      }
    }
  }

  private handleServerNotification(notification: JsonRpcNotification) {
    const event = {
      method: notification.method,
      params: notification.params as JsonValue,
  ...
  private async resolveServerRequest(request: JsonRpcServerRequest): Promise<Record<string, unknown>> {
  ...
  private async request(method: string, params: Record<string, unknown>): Promise<unknown> {
    ...
    console.info("[webui] app-server:client-response", summarizeJsonRpcResponse(response, method, id));
    console.info(
...
export async function createAppServerBrowserRuntime(
  ...
  const runtime = new runtimeModule.WasmBrowserRuntime(host) as WasmProtocolRuntime;
  await runtime.send({
    jsonrpc: "2.0",
    id: "initialize",
    method: "initialize",
  ...
  return new AppServerBrowserRuntime(runtime);
}
...
function isJsonRpcNotification(value: JsonValue): value is JsonRpcNotification {
  return (
    value !== null &&
    typeof value === "object" &&
    !Array.isArray(value) &&
...
function isJsonRpcRequest(value: JsonValue): value is JsonRpcServerRequest {
  return (
    value !== null &&
    typeof value === "object" &&
    !Array.isArray(value) &&
...
function summarizeJsonRpcNotification(notification: JsonRpcNotification): Record<string, unknown> {
  return {
    method: notification.method,
    keys: Object.keys(notification.params ?? {}),
    turnId: extractTurnId(notification.params),
  };
}

function summarizeJsonRpcRequest(request: JsonRpcServerRequest): Record<string, unknown> {
  return {
    id: request.id,
    method: request.method,
...
function summarizeJsonRpcResponse(
