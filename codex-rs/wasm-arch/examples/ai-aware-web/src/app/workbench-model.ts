...
export function createWorkbenchModel(input: {
  ...
  const metrics = buildMetrics(input.uiSystem.widgets.metrics.items, {
    ...
    view: input.uiSystem.activeView?.name ?? "Mission",
    dashboard: input.uiSystem.activeDashboard?.name ?? "Mission",
  ...
  return {
    ...
    sessionStatus: buildSessionStatus(
      ...
      input.uiSystem.activeView?.name ?? "Mission",
      input.uiSystem.activeDashboard?.name ?? "Mission",
    ),
    metrics,
    toolActivities,
