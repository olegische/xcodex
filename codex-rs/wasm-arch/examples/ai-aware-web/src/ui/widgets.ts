...
export const DEFAULT_UI_WIDGETS: UiWidgetsDocument = {
  ...
  composer: {
    placeholder: "What should I do in this browser environment?",
    position: "bottom",
  },
  shell: {
...
function buildWidgetsGuide(): string {
  return [
    ...
    "- `remote_mcp` reads `/workspace/ai-aware/mcp-servers.json`",
    "- `web_signals` reads `/workspace/ai-aware/web-signals.json`",
    "- `agent_swarm` reads `/workspace/ai-aware/swarm.json`",
    "",
  ].join("\n");
}
