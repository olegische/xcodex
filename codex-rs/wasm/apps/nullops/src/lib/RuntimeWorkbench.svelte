<script lang="ts">
  ...
  import NullopsPreviewPane from "./NullopsPreviewPane.svelte";
  ...
  $: displayTranscript =
    runtimeUiState.transcriptEntries.length > 0
      ...
      : state.transcript;
  $: nullopsAppFile =
    workspaceFiles.find((file) => file.path === "/workspace/nullops/app.html") ??
    workspaceFiles.find((file) => file.path === "/workspace/nullops/index.html") ??
    null;
  $: showNullopsPreview = nullopsAppFile !== null;
</script>
...
<AppShell {drawerOpen} sidebarOpen={inspectorState.sidebarOpen} sidebarSide={renderPlan.sidebarSide}>
  ...
  <div slot="main" class="main-column">
    ...
    <div class="main-workspace">
      {#if missionMode}
        <section class:chat-screen={!showNullopsPreview} class:mission-studio={showNullopsPreview}>
          <div class:chat-screen-body={!showNullopsPreview} class:mission-studio-chat={showNullopsPreview}>
            <ChatFoundation
          ...
          {#if showNullopsPreview}
            <div class="mission-studio-preview">
              <NullopsPreviewPane {workspaceFiles} />
            </div>
          {/if}
        </section>
      {:else}
