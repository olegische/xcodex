...
export function readManifestExcerpt(workspaceFiles: WorkspaceFileSummary[]): string {
  ...
  if (file === undefined) {
    return "NullOps browser runtime with anchored artifacts, citations, live app output, and replayable state.";
  }
  const excerpt = file.content
    ...
    .slice(0, 2)
    .join(" ");
  return excerpt || "NullOps browser runtime with anchored artifacts, citations, live app output, and replayable state.";
}
...
function buildManifestDocument(): string {
  return [
    "# NullOps",
    "",
    "This workspace is the source of truth for the NullOps browser runtime.",
    "",
    "Design constraints:",
