<script lang="ts">
  ...
  function activitySummary(activity: RuntimeActivity): string {
    ...
    if (activity.type === "apsixZone") {
      return `${activity.lifecycleState} / ${activity.phase}: ${activity.summary}`;
    }
    if (activity.type === "apsixSpawn") {
      return `spawn ${activity.actorId} -> ${activity.decision}`;
    }
    if (activity.type === "apsixArtifact") {
      return `artifact ${activity.artifactId} -> ${activity.status}`;
    }
    if (activity.type === "apsixAnchor") {
      return `anchor ${activity.anchorId} -> ${activity.decision}`;
    }
    if (activity.type === "apsixFreeze") {
      return `freeze ${activity.zoneId}`;
    }
