...
export function createWorkbenchModel(input: {
  ...
  const metrics = buildMetrics(input.uiSystem.widgets.metrics.items, {
    ...
    view: input.uiSystem.activeView?.name ?? "Default View",
    dashboard: input.uiSystem.activeDashboard?.name ?? "Default Dashboard",
  ...
  return {
    ...
    sessionStatus: buildSessionStatus(
      ...
      input.uiSystem.activeView?.name ?? "Default View",
      input.uiSystem.activeDashboard?.name ?? "Default Dashboard",
    ),
    metrics,
    toolActivities,
