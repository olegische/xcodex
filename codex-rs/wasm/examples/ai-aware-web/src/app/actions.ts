...
import { ensureAiAwareWorkspaceSeed } from "../aiAware/workspace";
...
export async function initializeUiShell() {
  ...
  await ensureAiAwareWorkspaceSeed();
...
export async function sendTurn(): Promise<void> {
  ...
  try {
    ...
    missionControlStore.startMission(message);
    ...
    if (stopRequested || isCancellationError(error)) {
      missionControlStore.block("Mission stopped by user.");
      runtimeSessionStore.setCancelledStatus();
      return;
    }
    console.error("[webui] ui:send:failed", error);
    missionControlStore.block(formatError(error));
...
export async function stopTurn(): Promise<void> {
  ...
  try {
    ...
    runtimeUiStore.markCancelled();
    missionControlStore.block("Mission cancelled during execution.");
    runtimeSessionStore.setCancelledStatus();
  } catch (error) {
