...
export function setupAppLifecycle(options?: {
  ...
  void (async () => {
    try {
      ...
      await runBootStep("mission_control", "Loading mission control", async () => {
        disconnectMissionControl = missionControlStore.initialize();
      });
...
function logRuntimeActivity(
  ...
  if (activity.type === "missionState") {
    console.info("[webui] mission-state", activity);
    return;
  }
