...
use codex_wasm_v2_app_server::process_core_event as process_in_process_core_event;
use codex_wasm_v2_app_server::resolve_server_request;
...
impl WasmBrowserRuntime {
    ...
    async fn handle_thread_start(
...

        {
            ...
            state.threads.insert(
                thread_id.clone(),
                LoadedThread {
                    app_server: MessageProcessor::new(MessageProcessorArgs {
    ...
    async fn handle_server_response(
        ...
        let resolved = resolve_server_request(pending, result)
            .map_err(|error| JsValue::from_str(&error.to_string()))?;
        let (codex, thread_id) = {
            let state = self.state.lock().await;
            let loaded = state
                .threads
                .get(&resolved.thread_id)
                .ok_or_else(|| JsValue::from_str("thread missing for pending server request"))?;
            (Arc::clone(&loaded.codex), resolved.thread_id)
        };
...
async fn process_core_event(
    ...
    let mut outgoing_requests = Vec::new();
    {
        ...
        loaded.record.updated_at = now_unix_seconds();
        let mut thread = loaded.record.in_process_thread_handle();
        let effect = process_in_process_core_event(
            &mut loaded.app_server,
            thread_id,
            &mut thread,
            next_request_id,
            &event,
        );
        loaded.record.apply_in_process_thread_handle(thread);
        ...
        match &event.msg {
            ...
            | EventMsg::ElicitationRequest(_)
            | EventMsg::DynamicToolCallRequest(_) => {
                for (request_id, pending, request) in effect.server_requests {
