import type { StoredUserConfig } from "./types/config.ts";
import type {
  BrowserRuntimeStorage,
  CreateIndexedDbCodexStorageOptions,
} from "./types/runtime.ts";

type IndexedDbRuntimeStoreNames = {
...
>(
  ...
  async function openDb(): Promise<IDBDatabase> {
    return await new Promise((resolve, reject) => {
      ...
      request.onerror = () =>
        reject(request.error ?? new Error(`failed to open indexeddb ${options.dbName}`));
    });
  }
  ...
  async function loadSession(threadId: string): Promise<TSession | null> {
    ...
    return await new Promise((resolve, reject) => {
      ...
      request.onerror = () =>
        reject(request.error ?? new Error("failed to load thread session"));
    });
  }
  ...
  async function saveSession(session: TSession): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const request = tx
        .objectStore(storeNames.threadSessions)
        .put(session, options.getSessionId(session));
      request.onsuccess = () => resolve();
      request.onerror = () =>
        reject(request.error ?? new Error("failed to save thread session"));
    });
  }
  ...
  async function deleteSession(threadId: string): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      request.onsuccess = () => resolve();
      request.onerror = () =>
        reject(request.error ?? new Error("failed to delete thread session"));
    });
  }
  ...
  async function listSessions(): Promise<TSessionMetadata[]> {
    ...
    return await new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        const sessions = Array.isArray(request.result)
          ? (request.result as TSession[])
          : [];
        resolve(sessions.map((session) => options.getSessionMetadata(session)));
      };
      request.onerror = () =>
        reject(request.error ?? new Error("failed to list thread sessions"));
    });
  }
  ...
  async function loadAuthState(): Promise<TAuthState | null> {
    ...
    return await new Promise((resolve, reject) => {
      ...
      request.onsuccess = () =>
        resolve((request.result as TAuthState | undefined) ?? null);
  ...
  async function clearAuthState(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      request.onsuccess = () => resolve();
      request.onerror = () =>
        reject(request.error ?? new Error("failed to clear auth state"));
    });
  }
      ...
      request.onsuccess = () =>
        resolve(
          options.normalizeConfig(
            (request.result as TConfig | undefined) ??
              structuredClone(options.defaultConfig),
          ),
        );
      request.onerror = () =>
        reject(request.error ?? new Error("failed to load provider config"));
    });
  }
  ...
  async function saveConfig(config: TConfig): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      request.onsuccess = () => resolve();
      request.onerror = () =>
        reject(request.error ?? new Error("failed to save provider config"));
    });
  }
  ...
  async function clearConfig(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      request.onsuccess = () => resolve();
      request.onerror = () =>
        reject(request.error ?? new Error("failed to clear provider config"));
    });
  }
  ...
  async function loadUserConfig(): Promise<StoredUserConfig | null> {
    ...
    return await new Promise((resolve, reject) => {
      ...
      request.onsuccess = () =>
        resolve((request.result as StoredUserConfig | undefined) ?? null);
      request.onerror = () =>
        reject(request.error ?? new Error("failed to load user config"));
    });
  }
